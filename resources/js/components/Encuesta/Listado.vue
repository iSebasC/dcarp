        ...
        <div class="row breadcrumbs-top d-block">
          <div class="breadcrumb-wrapper col-12">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active">Encuestas de Trabajo</li>
            </ol>
          </div>
        </div>
      </div>
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h4 class="card-title">Listado de Encuestas de Trabajo</h4>
            </div>
                ...
                <table class="table mb-0">
                  <thead>
                    <tr>
                      ...
                      <th class="text-center">¿Se aplicó encuesta?</th>
                  ...
                  <tbody>
                    ...
                    <tr
                      ...
                      :key="p.id"
                      :class="
                        p.puntuacionEncuesta > 0 && p.puntuacionEncuesta <= 13
                          ? 'table-danger'
                          : p.puntuacionEncuesta > 13 &&
                            p.puntuacionEncuesta <= 16
                          ? 'table-warning'
                          : p.puntuacionEncuesta > 16 &&
                            p.puntuacionEncuesta <= 20
                          ? 'table-success'
                          : ''
                      "
                    >
                      ...
                      <td class="text-center">
                        <strong v-text="p.documento"></strong>
                      </td>
                      <td class="text-center">
                        <strong v-text="p.documentocita"></strong>
                      </td>
                      ...
                      <td class="text-center">
                        <span
                          :class="
                            'badge badge-pill ' +
                            (p.situacionEncuesta == 'Sin Aplicar'
                              ? 'badge-danger'
                              : 'badge-info')
                          "
                          v-text="p.situacionEncuesta"
                        >
                        </span>
                      </td>
                      ...
                      <td class="text-center">
                        <div class="content-buttons btn-group">
                          <button
                            v-if="p.situacionEncuesta == 'Sin Aplicar'"
                            @click="
                              llenarEncuestaF(
                                p.id,
                                p.documento,
                          ...
                          >
                            <i class="mdi mdi-set-none icon-size"></i>
                          </button>

                          <button
                            @click="verFormato(p.id)"
                            title="Imprimir Formatos en PDF"
                            class="btn btn-sm btn-outline-danger"
                          >
                            <i class="mdi mdi-printer-check icon-size"></i>
                          </button>
                        </div>
                      </td>
                    </tr>
...
import Encuesta from "../RevisionOrden/Encuesta";
...
export default {
  ...
  methods: {
    ...
    async changeBackground() {
      let elem = document.getElementById("ptje");
      let ptje = elem.value;
      elem.style.background = ptje;
      await this.busquedaOrden();
    },
    async busquedaOrden() {
      let me = this;
      ...
      let colorPtje = document.getElementById("ptje").value;
    ...
    excel() {
      ...
      window.open(
        `/encuesta/excel?fi=${fechaI}&ff=${fechaF}&doc=${documento}&cli=${cliente}&color=${colorPtje.slice(
          1
        )}&cita=${comprobante_cita}&comp=${comprobante}&reg=${registrado_por}&placa=${placa_vehiculo}`,
        "_blank"
      );
    },
    llenarEncuestaF(id, documento, cliente, placa, correo, telefono, dets) {
      let me = this;
      me.isValidSession();
      let arrDetalles = dets.split('@@');
      let marcamodelo = arrDetalles[0];
      let kilometraje = arrDetalles[1];
      let vin = arrDetalles[2];
      
      this.$store.state.mostrarModal = !this.$store.state.mostrarModal;
      me.$refs.encuesta.showModal(
        id,
        documento,
        cliente,
        placa,
    ...
    verFormato(id) {
      window.open(`/getencuesta/${id}`, "_blank");
    },
  },
  created() {
