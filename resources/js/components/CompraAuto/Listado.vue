    ...
    <div class="content-header row">
        ...
        <div class="row breadcrumbs-top d-block">
          <div class="breadcrumb-wrapper col-12">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active">Compras de Autos
              </li>
            </ol>
          </div>
        </div>
      ...
      <div class="content-header-right col-md-6 col-12">
        <div class="btn-group float-md-right" role="group" aria-label="Agregar Compra">
          <router-link tag="a" to="/compraauto/crear" class="btn btn-info btn-sm btn-rounded box-shadow-2 px-2 mb-1"
           type="button"><i class="mdi mdi-plus icon-size"></i> Agregar Compra</router-link>
        </div>
      </div>
    </div>
          ...
          <div class="col-12">
              <div class="card">
                  <div class="card-header">
                      <h4 class="card-title">Listado de Compras de Autos</h4>
                  </div>
                            ...
                            <div class="row">
                              ...
                              <div class="col-md-2 pt-xs-1 pl-xs-2">
                                <div class="content-buttons btn-group">
                                  <button type="button" class="btn btn-sm btn-icon btn-md btn-info mb-1" title="Buscar" @click="busquedaCompra"><i class="mdi mdi-magnify"></i></button>
                                  
                                  <!-- <button type="button" @click="excel" class="btn btn-sm btn-icon btn-success mb-1" title="Exportar a Excel"><i class="mdi mdi-file-excel "></i></button> -->
                                </div>
                                ...
                                <div class="form-group row">
                                  ...
                                  <div class="col-md-5 pt-1 pl-0">
                                    <select class="custom-select custom-select-sm pr-2 ml-1" :id="'cantidad_'+entidad" title="Registros por Página" @change="busquedaCompra">
                      ...
                      <div class="table-responsive px-2 d-none" :id="'tabla_'+entidad">
                          <table class="table mb-0">
                            ...
                            <tbody>
                                ...
                                <tr v-for="(p, index) in compras" :key="p.id">
                                    ...
                                    <td class="text-center" v-text="p.fechaVencimiento"></td>
                                    <td class="text-center" v-text="p.flete"></td>
                                    <td class="text-center" v-text="p.total"></td>
                                    ...
                                    <td class="text-center">
                                      <div class="content-buttons btn-group">
                                        ...
                                        <button @click="anular(p.id,p.documento, p.proveedor)" title="Anular Compra" 
                          ...
                          <nav class="pl-2">
                            <ul class="pagination justify-content-right">
                              ...
                              <li v-for="(op,index) in opciones" :key="index" :class="(op.opc==pageActual?'page-item active':'page-item')" @click="(op.opc!=pageActual?buscarCompra(op.opc):'')" >
...
export default {
  ...
  methods: {
    ...
    excel () {
      ...
      // window.open(`/venta/excel?documento=${documento}&proveedor=${proveedor}&flete=${flete}&comprobante=${comprobante}&fechaI=${fechaI}
      // &fechaF=${fechaF}&moneda=${moneda}&tipodoc=${tipoDoc}`,'_blank')
    ...
    verPdf(id) {
      if (id != null) {
          miVentana = window.open("https://fasteinvoice.com/consultar.php?ruc=20603144954&password=valentinasunat123&id="+id,"Impresión de Comprobante Electrónico","scrollbars=1,status=0,titlebar=0,toolbar=0,resizable=1,width=1200, height=800")
          miVentana.print()
      }
    },
    anular (id,documento,proveedor) {
      ...
      swal({
        ...
        html: '¿Seguro que desea Eliminar la Compra <strong>' + me.documento + 
        '</strong> del Proveedor <strong>'+me.proveedor+'</strong>?<br><small style="color:red;"><strong>NOTA IMPORTANTE</strong><br>Compra no debe tener movimientos de stock en sus detalles.</small> ',
        type: 'warning',
        ...
        if (result.value) {
          let response = await axios({
            method: 'post',
            url: `/eliminarcompraauto/${me.idVenta}`,
            data: {
              '_token': token
            }
          })
      ...
      // me.proveedor = proveedor
      // this.$store.state.mostrarModal = !this.$store.state.mostrarModal
      // me.$refs.notacredito.showModal(id)
    },
  },
  created() {
