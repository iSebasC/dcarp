  ...
  <div class="content-wrapper" :id="'sec-personal_'+entidad">
    <div class="content-header row">
      ...
      <div class="content-header-right col-md-6 col-12">
        <div class="btn-group float-md-right" role="group" aria-label="Agregar Personal">
          <router-link tag="a" to="/personal/crear" class="btn btn-info btn-sm btn-rounded box-shadow-2 p-x-2 mb-1" 
          type="button"><i class="mdi mdi-plus icon-size"></i> Agregar Personal</router-link>
        </div>
      </div>
    </div>
                        ...
                        <fieldset class="form-group">
                            <div class="row">
                                    ...
                                    <div class="row">
                                      <div class="col-sm-12">
                                        <div class="form-group row">
                                          <label :for="'dni_'+entidad" class="col-sm-1 m-4px col-form-label pr-0 mr-0 pt-0">DNI:</label>
                                          <div class="col-sm-2">
                                              <input type="text" name="dni" class="form-control form-control-sm" :id="'dni_'+entidad" 
                                              @keyup.enter="busquedaPersonal" />
                                          </div>

                                          <label :for="'personal_'+entidad" class="col-sm-1 m-4px col-form-label pr-0 mr-0 pt-0">Personal:</label>
                                          <div class="col-sm-4">
                                              <input type="text" name="personal" class="form-control form-control-sm" :id="'personal_'+entidad" 
                                              @keyup.enter="busquedaPersonal" placeholder="Apellidos y Nombres" />
                                          </div>

                                          <label :for="'telefono_'+entidad" class="col-sm-1 m-4px col-form-label pr-0 mr-0 pt-0">Teléfono:</label>
                                          <div class="col-sm-2">
                                              <input type="text" name="telefono" @keypress="soloNumeros($event)" minlength="6" maxlength="9" class="form-control form-control-sm" :id="'telefono_'+entidad" 
                                              @keyup.enter="busquedaPersonal" />
                                          </div>
                                        </div>   

                                        <div class="form-group row">
                                          <label :for="'correo_'+entidad" class="col-sm-2 m-4px col-form-label pr-0 mr-0 pt-0">C. Electrónico:</label>
                                          <div class="col-sm-3">
                                              <input type="text" name="correo" class="form-control form-control-sm" :id="'correo_'+entidad" 
                                              @keyup.enter="busquedaPersonal" />
                                          </div>

                                          <label :for="'direccion_'+entidad" class="col-sm-2 m-4px col-form-label pr-0 mr-0 pt-0 text-md-center">Dirección:</label>
                                          <div class="col-sm-4">
                                        ...
                                        <div class="form-group row">
                                          ...
                                          <div class="col-sm-2">
                                            <select class="custom-select custom-select-sm" :id="'genero_'+entidad" @change="busquedaPersonal">
                                          ...
                                          <div class="col-sm-3">
                                            <select class="custom-select custom-select-sm" :id="'categoria_'+entidad" @change="busquedaPersonal">
                                              <option value="todo">Todos</option>
                                ...
                                <div class="col-md-2 pt-xs-1 pl-xs-2">
                                    ...
                                    <button type="button" class="btn btn-sm btn-icon btn-info mb-1" title="Buscar" @click="busquedaPersonal"><i class="mdi mdi-magnify"></i></button>

                                    <!--<button type="button" class="btn btn-xs btn-icon btn-danger mb-1" title="Exportar a Pdf"><i class="la la-file-pdf"></i></button>-->

                                    <button type="button" class="btn btn-sm btn-icon btn-success mb-1" title="Exportar a Excel" @click="excel"><i class="mdi mdi-file-excel "></i></button>
                                    </div>

                                    <div class="form-group row">
                                      <label class="col-md-5 label-control pt-1 pl-1 pr-0" :for="'cantidad_'+entidad" style="margin-top:3px;">Mostrar: </label>
                                      <div class="col-md-5 pt-1 pl-0">
                                        <select class="custom-select custom-select-sm pr-2 ml-1" :id="'cantidad_'+entidad" title="Registros por Página" @change="busquedaPersonal">
                      ...
                      <div class="table-responsive px-2 d-none" :id="'tabla_'+entidad">
                          <table class="table mb-0">
                            ...
                            <tbody>
                                ...
                                <tr v-for="(p, index) in personal" :key="p.id">
                                    ...
                                    <td class="text-center" v-text="p.dni"></td>
                                    <td class="text-center" v-text="p.personal"></td>
                                    <td class="text-center" v-text="p.genero"></td>
                                    <td class="text-center" v-text="p.fechaNacimiento"></td>
                                    <td class="text-center" v-text="p.correoE"></td>
                                    <td class="text-center" v-text="p.telefono"></td>
                                    <td class="text-center" v-text="p.direccion"></td>
                                    <td class="text-center" v-text="p.tipoUsuario"></td>
                                    <td class="text-center">
                                      <div class="content-buttons btn-group">
                                        <button @click="generarHabilidades(p.id, p.personal)" title="Gestión de Habilidades" class="btn btn-sm btn-primary">
                                          <i class="mdi mdi-apple-keyboard-command icon-size"></i>
                                        </button>
                          ...
                          <nav class="pl-2">
                            <ul class="pagination justify-content-right">
                              ...
                              <li class="page-item" @click="((paramInicio < pageActual && paramInicio > 0)?buscarPersonal('prev'):'')">
                                <a class="page-link" href="javascript:void(0);" aria-label="Previous">«</a>
                              </li>

                              <li v-for="(op,index) in opciones" :key="index" :class="(op.opc==pageActual?'page-item active':'page-item')" @click="(op.opc!=pageActual?buscarPersonal(op.opc):'')" >
                                  <a class="page-link" href="javascript:void(0);" v-text="op.opc" :disabled="op.bloqueado"></a>
                              </li>
                              <li class="page-item" @click="((paramFin > pageActual && paramFin >0)?buscarPersonal('next'):'')">
    ...
    <Habilidades ref="habilidades" :idmodal="this.idPersonal" :personal="this.persona"></Habilidades>
...
export default {
  ...
  data() {
    return {
      ...
      generos: [{'id': 'F', 'nombre': 'Femenino'}, {'id': 'M', 'nombre': 'Masculino'}],
      total: '',
      inicio: '',
      fin: '',
      paramInicio:'',
  ...
  methods: {
    ...
    excel () {
      ...
      window.open(`/personal/excel?dni=${dni}&personal=${personal}&correo=${correo}&direccion=${direccion}&telefono=${telefono}&genero=${genero}&categoriaId=${categoriaId}`, '_blank')
