                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-6 col-xs-12">
                                                            <div class="form-group">
                                                                ...
                                                                name="documento" maxlength="12" minlength="8" @keypress="soloLetrasNumerosDocumento($event)" 
                                                        ...
                                                        <div class="col-md-5 col-xs-12">
                                                            <div class="form-group">
                                                                ...
                                                                class="custom-select custom-select-sm">
                                                                    <option v-for="item in arrMonedas" :selected="monedaId == item.id" :key="item.id" 
                                                                        :value="item.id">
                                                                        {{ item.nombre }}
                                                                    </option>
                                                                </select>
...
export default {
 ...
 data () {
     return {
         ...
         min:1,
         max:10000,
         entidad: 'convert_cotizacion_auto',
         arrMonedas: [
            {id: 'D', nombre: 'Dólar'},
            {id: 'S', nombre: 'Soles'}
         ],
 ...
 methods: {
    ...
    async cargarDatos () {
      let me = this
      me.accion = 'Registrar'
      me.oportunidadId = (this.$route.params.oportunidadId == undefined?0:this.$route.params.oportunidadId)
    ...
    async completeData() {
        let me = this
        let response = await axios.get(`/findoportunidad/${me.oportunidadId}`);
        if (response.data.estado) {
            let oportunidad = response.data.oportunidad;
            document.getElementById(`documento_${me.entidad}`).value = oportunidad.documento;
            document.getElementById(`tipoMoneda_${me.entidad}`).value = (oportunidad.moneda == 'USD'?'D': 'S');

            me.monedaId = document.getElementById(`tipoMoneda_${me.entidad}`).value;
            me.monedaSeleccionada = document.getElementById(`tipoMoneda_${me.entidad}`).value;
            me.busquedaCliente();
            let id_aleatorio = me.generarCorrelativo() // me.generarAleatorio(me.min, me.max, me.listDetalles) 
            let elemento = { id: id_aleatorio, tipo: 'Auto', precio: oportunidad.monto, 
                             descripcion: oportunidad.concepto, stock: 1, cantidad: 1, stockMax: 1, 
                             tiempo: 0, idP: oportunidad.idAuto };
    ...
    async busquedaCliente () {
        ...
        if (valorDoc.length >= 8 && valorDoc.length <= 12) { 
    ...
    capturarDato (ref) {
        ...
        // alert(ref.nombre)
        let rpta = me.buscarInArray(ref.id, 'Auto')
        document.getElementById(`data-stock_${me.entidad}`).innerHTML = ''
    ...
    agregarDetalle() {
        ...
        if (ref != null) {
            ...
            descripcion: ref.descripcionadicional, stock: ref.stock, cantidad: cantidad, stockMax: me.stockMax, tiempo: ref.tiempo,idP: ref.id }
        } else {
