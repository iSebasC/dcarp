                  ...
                  >
                    ...
                    <div class="form-body">
                      ...
                      <div class="row">
                        ...
                        />
                        <input type="hidden" name="ref_pedido" :value="id">
                        <div
                          class="table-responsive px-1"
                    ...
                    <div class="form-actions ml-1 mt-3">
                      <button
                        type="button"
...
export default {
  name: "MantenimientoPedidoCompraConvert",
  mixins: [misMixins],
  components: {
    Proveedor,
  },
  ...
  methods: {
    ...
    async cargarDatos() {
      let me = this;
      me.accion = "Registrar";
      me.id = (this.$route.params.pedidoId == undefined?0:this.$route.params.pedidoId)
      ...
      document.getElementById("fechavencimiento_" + me.entidad).value =
        me.obtenerFechaActual();
      if (me.id > 0) {
        let response = await axios.get("/obtenerpedidocompra/" + me.id);
        window.setTimeout(function () {
          if (response.data.estado) {
            let band = false;
            let bandErrors = false;
            let fleteU = 0;
            let pedido = response.data.pedido;
            let detalles = response.data.detalles;
            let errors = 'Se encontraron servicios en su Pedido de Compra: <br>';
            document.getElementById(`documento_${me.entidad}`).value = pedido.documento;
            document.getElementById(`cliente_${me.entidad}`).value = pedido.proveedor;
            document.getElementById(`tipoMoneda_${me.entidad}`).value = pedido.tipoMoneda;
            document.getElementById(`credito_${me.entidad}`).value = pedido.diasCredito;

            me.documento = pedido.documento;
            me.tipo = (pedido.tipoMoneda == 'S'? pedido.tipoMoneda + '/':  '$');
            detalles.forEach(ref => {
              band = false;
              if (ref.idProducto != null) {
                let ax = parseFloat(ref.total) / (1 - parseFloat(ref.porcentaje)/100) + fleteU * parseFloat(ref.cantidad);
                let precio_venta = Math.round((ax / parseFloat(ref.cantidad)) * 100) / 100;
          
                var elemento = {
                  id: ref.id,
                  tipo: 'Producto',
                  descripcion: ref.descripcion,
                  cantidad: parseFloat(ref.cantidad),
                  precio: parseFloat(ref.precio),
                  porcentaje: parseFloat(ref.porcentaje),
                  precioventa: precio_venta,
                };
                me.listProductos.push(ref.id);
                band = true;
              } else {
                errors += `Servicio: ${ref.descripcion}, item: ${ref.item}<br>`;
                bandErrors = true;
              }
              ...
              if (band) {
                ...
                me.calcularTotal();
                // me.deshabilitado = true; // REFERENTE A FLETE
              }
            })

            if (bandErrors) {
              errors += `<strong class="text-danger">Recuerde que los Servicios no se consideran para las Compras.</strong>`
              swal('¡Advertencia!', errors, 'warning');
            }
