...
where dcot.idProducto = dv.idProducto and `vt`.`idAlmacenSalida` = almacenId and `dv`.`idProducto` = productoId and `pd`.`idCotizacion` is not null and `pd`.`idOrden` is null
...
(CASE an.situacion WHEN 'V' THEN 'A' ELSE 'E' END) estado, (CASE an.tipoNota WHEN 'C' THEN 'NOTA DE CR�DITO' ELSE 'NOTA DE D�BITO' END) as tipoDoc, (SELECT GROUP_CONCAT(c.tipoMoneda,'-', dc.preciocompra) FROM compra as c JOIN detallecompra as dc ON c.id = dc.idCompra 
JOIN lote as l ON l.idCompra = c.id WHERE dc.idProducto = l.idProducto AND 
l.id = dv.idLote AND l.idProducto = dv.idProducto) as det_compra, (SELECT GROUP_CONCAT(g.tipoMoneda,'-', dg.preciocompra) FROM guia as g JOIN detalleguia as dg ON g.id = dg.idGuia 
...
(SELECT GROUP_CONCAT(c.tipoMoneda,'-', dc.preciocompra* c.tipoCambio) FROM compraauto as c JOIN detallecompraauto as dc ON c.id = dc.idCompra 
...
JOIN venta as v ON v.id = an.idVenta
JOIN persona as cl ON cl.id = v.idCliente
WHERE dtan.idServicio IS NULL AND dtan.idProducto IS NULL AND dtan.idAuto IS NULL;
END ;;
DELIMITER ;
...
DELIMITER ;;
CREATE PROCEDURE `listarDetalleVentaAutoRechazoSUNAT`()
    NO SQL
    COMMENT 'Lista ventas de autos rechazadas por SUNAT'
CREATE TEMPORARY TABLE tmp_auto_rechazo_sunat AS SELECT ma.nombre as macro, ma.nombre as tipo, CONCAT(ma.nombre,'-', p.version,'-', p.transmision,'-', p.descripcion) as nombre,
  dv.descripcion, CONCAT(ma.nombre,'-', p.version,'-', p.transmision,'-', p.descripcion) as tipoProducto, cl.documento as docCliente,
  (CASE WHEN cl.razonSocial IS NULL THEN CONCAT(cl.apellidos, ' ', cl.nombres) ELSE cl.razonSocial END ) as cliente,
  dv.cantidad, (dv.precio * v.tipoCambio) as precio, (dv.subtotal * v.tipoCambio) as subtotal,
  v.igv_sunat, (v.igv * v.tipoCambio) as igv, (v.total * v.tipoCambio) as total, DATE_FORMAT(v.fecha, '%d/%m/%Y') as fecha,
  dv.created_at, v.tipoComprobante, CONCAT( v.tipoComprobante, LPAD(v.serie, 3, '0'), '-', LPAD(v.numero, 8, '0')) as documento, 
  v.situacion,
  v.tipoMoneda as moneda, (SELECT COUNT(dvt.id) as cantidad FROM   detalleventa dvt WHERE   dvt.idVenta = v.id ) as cant_items, ( SELECT GROUP_CONCAT(c.tipoMoneda, '-', (dc.preciocompra * c.tipoCambio)) FROM   compraauto as c   JOIN detallecompraauto as dc ON c.id = dc.idCompra   JOIN loteauto as l ON l.idCompra = c.id WHERE   dc.idAuto = l.idAuto   AND l.id = dv.idLoteAuto   AND l.idAuto = dv.idAuto
  ) as det_compra, (SELECT GROUP_CONCAT(g.tipoMoneda, '-', (dg.preciocompra*g.tipoCambio)) FROM   guia as g   JOIN detalleguia as dg ON g.id = dg.idGuia   JOIN loteauto as l ON l.idGuiaCompra = g.id WHERE   dg.idAuto = l.idAuto   AND l.id = dv.idLoteAuto   AND l.idAuto = dv.idAuto
  ) as det_guia
FROM venta as v
  JOIN detalleventa as dv ON dv.idVenta = v.id
  LEFT JOIN pagodetalle as pd ON dv.id = pd.idDetalleVenta
  JOIN auto as p ON p.id = dv.idAuto
  JOIN marcaauto as ma ON ma.id = p.marcaId
  JOIN persona as cl ON cl.id = v.idCliente
WHERE pd.idVenta IS NULL AND v.situacionSUNAT = 'R' ;;
DELIMITER ;
...
DELIMITER ;;
CREATE PROCEDURE `listarDetalleVentaProductoRechazoSUNAT`()
    NO SQL
    COMMENT 'Lista ventas directas de productos con rechazo SUNAT'
BEGIN
CREATE TEMPORARY TABLE tmp_producto_rechazo_sunat AS 
SELECT tp.nombre as macro, p.nombre as tipo, p.nombre, dv.descripcion, p.tipoProducto, cl.documento as docCliente, (CASE WHEN cl.razonSocial IS NULL THEN CONCAT(cl.apellidos,' ', cl.nombres) ELSE cl.razonSocial END) as cliente, dv.cantidad, (dv.precio * v.tipoCambio) as precio, (dv.subtotal * v.tipoCambio) as subtotal, v.igv_sunat, (v.igv * v.tipoCambio) as igv, (v.total * v.tipoCambio) as total, DATE_FORMAT(v.fecha,'%d/%m/%Y') as fecha, dv.created_at, v.tipoComprobante, CONCAT(v.tipoComprobante, LPAD(v.serie,3,'0') ,'-', LPAD(v.numero,8,'0')) as documento, 
v.situacion,
v.tipoMoneda as moneda, 
(SELECT COUNT(dvt.id) as cantidad FROM detalleventa dvt WHERE dvt.idVenta = v.id) as cant_items,
...
FROM venta as v
JOIN detalleventa as dv ON dv.idVenta = v.id
LEFT JOIN pagodetalle as pd ON dv.id = pd.idDetalleVenta AND pd.idVenta = v.id
JOIN producto as p ON p.id = dv.idProducto
JOIN persona as cl ON cl.id = v.idCliente
JOIN tipoproducto as tp ON tp.id = p.idMargenGanancia
WHERE pd.idVenta IS NULL AND v.situacionSUNAT = 'R';
...
DELIMITER ;;
CREATE PROCEDURE `listarDetalleVentaServicioRechazoSunat`()
BEGIN
CREATE TEMPORARY TABLE tmp_servicio_rechazo_sunat AS 
SELECT ms.descripcion as macro, cats.nombre as tipo, s.nombre, dv.descripcion, s.nombre as tipoProducto, cl.documento as docCliente, (CASE WHEN cl.razonSocial IS NULL THEN CONCAT(cl.apellidos,' ', cl.nombres) ELSE cl.razonSocial END) as cliente, dv.cantidad, (dv.precio * v.tipoCambio) as precio, (dv.subtotal * v.tipoCambio) as subtotal, v.igv_sunat, (v.igv * v.tipoCambio) as igv, (v.total * v.tipoCambio) as total, DATE_FORMAT(v.fecha,'%d/%m/%Y') as fecha, dv.created_at, v.tipoComprobante, CONCAT(v.tipoComprobante, LPAD(v.serie,3,'0') ,'-', LPAD(v.numero,8,'0')) as documento, 
v.situacion, 
v.tipoMoneda as moneda,
(SELECT COUNT(dvt.id) as cantidad FROM detalleventa dvt WHERE dvt.idVenta = v.id) as cant_items,
NULL as det_compra, NULL as det_guia FROM venta as v JOIN detalleventa as dv ON dv.idVenta = v.id LEFT JOIN pagodetalle as pd ON dv.id = pd.idDetalleVenta AND pd.idVenta = v.id JOIN servicio as s ON s.id = dv.idServicio JOIN categoriaservicio as cats ON cats.id = s.idCategoriaServicio JOIN persona as cl ON cl.id = v.idCliente LEFT JOIN macroservicio as ms ON ms.id = s.idMacroServicio
WHERE pd.idVenta IS NULL AND v.situacionSUNAT = 'R';
END ;;
DELIMITER ;
...
BEGIN
CREATE TEMPORARY TABLE tmp_venta_ple AS 
SELECT CONCAT(DATE_FORMAT(v.fecha,'%Y%m'),'00') as campo1, CONCAT(DATE_FORMAT(fecha,'%m%y'),'V') as campo2, 
...
NULL as campo30,
NULL as campo31,
(CASE WHEN v.situacion = 'A' THEN (SELECT (CASE WHEN COUNT(an.id) > 0 THEN 'A' ELSE (CASE WHEN v.situacionSUNAT = 'R' THEN 'A' ELSE 'V' END) END) as situacion FROM anulacion as an WHERE an.idVenta = v.id) ELSE v.situacion END) as situacion,
v.created_at
FROM venta as v
JOIN persona as cl ON cl.id = v.idCliente;
END ;;
...
DELIMITER ;;
CREATE PROCEDURE `reporteClientesMarketing`(IN `fechaI` DATE, IN `fechaF` DATE, IN `cliente` VARCHAR(255))
BEGIN
	IF fechaI IS NOT NULL AND fechaF IS NOT NULL THEN
        SELECT DISTINCT cl.tipoPersona, cl.tipoDocumento, cl.documento, 
         (CASE cl.tipoDocumento WHEN 'PN' THEN CONCAT(cl.nombres,' ', cl.apellidos) 
                               ELSE  cl.razonSocial END) cliente, cl.telefono, 
         cl.correoElectronico,  ma.nombre marca, ct.modelo, ct.placa
        FROM ordentrabajo ot 
        LEFT JOIN cita ct ON ct.id = ot.idCita AND ct.placa = ot.placa
        JOIN marcaauto ma ON ma.id = ct.idMarcaAuto
        JOIN persona cl ON cl.id = ot.idCliente
        WHERE ot.fecha BETWEEN fechaI AND fechaF
        HAVING cliente LIKE cliente;
    ELSE 
    	IF fechaI IS NOT NULL THEN 
        	SELECT DISTINCT cl.tipoPersona, cl.tipoDocumento, cl.documento, 
             (CASE cl.tipoDocumento WHEN 'PN' THEN CONCAT(cl.nombres,' ', cl.apellidos) 
                                   ELSE  cl.razonSocial END) cliente, cl.telefono, 
             cl.correoElectronico,  ma.nombre marca, ct.modelo, ct.placa
            ...
            JOIN marcaauto ma ON ma.id = ct.idMarcaAuto
            JOIN persona cl ON cl.id = ot.idCliente
            WHERE ot.fecha >= fechaI
            HAVING cliente LIKE cliente;
        END IF;
        
        IF fechaF IS NOT NULL THEN 
        	SELECT DISTINCT cl.tipoPersona, cl.tipoDocumento, cl.documento, 
             (CASE cl.tipoDocumento WHEN 'PN' THEN CONCAT(cl.nombres,' ', cl.apellidos) 
                                   ELSE  cl.razonSocial END) cliente, cl.telefono, 
             cl.correoElectronico,  ma.nombre marca, ct.modelo, ct.placa
            ...
            JOIN marcaauto ma ON ma.id = ct.idMarcaAuto
            JOIN persona cl ON cl.id = ot.idCliente
            WHERE ot.fecha <= fechaF
            HAVING cliente LIKE cliente;
        END IF;
        
        IF fechaI IS NULL AND fechaF IS NULL THEN 
        	SELECT DISTINCT cl.tipoPersona, cl.tipoDocumento, cl.documento, 
             (CASE cl.tipoDocumento WHEN 'PN' THEN CONCAT(cl.nombres,' ', cl.apellidos) 
                                   ELSE  cl.razonSocial END) cliente, cl.telefono, 
             cl.correoElectronico,  ma.nombre marca, ct.modelo, ct.placa
...
BEGIN
		...
		CALL listarDetalleVentaProductoRechazoSUNAT();
        CALL listarDetalleVentaAutoRechazoSUNAT();
        CALL listarDetalleVentaServicioRechazoSunat();
        ...
        (SELECT * FROM tmp_otros_nota) UNION ALL
        (SELECT * FROM tmp_producto_rechazo_sunat) UNION ALL
        (SELECT * FROM tmp_auto_rechazo_sunat) UNION ALL
        (SELECT * FROM tmp_servicio_rechazo_sunat);
...
CONCAT((CASE WHEN producto.nombre IS NULL THEN '' ELSE producto.nombre END)) as nombre, (CASE WHEN tipoProducto = 'B' THEN CONCAT('Marca: ', mb.nombre, ', Modelo: ',modb.nombre,', Placa:', producto.placaBateria, ' - Tipo: ', (CASE WHEN producto.tipoBateria = 'L' THEN 'L�quida' ELSE 'Seca' END)) ELSE NULL END) as nombre2,
...
WHEN 'LL' THEN 'Neum�ticos' 
WHEN 'I'  THEN 'Insumos' 
WHEN 'B'  THEN 'Bater�as' 
...
WHERE producto.deleted_at IS NULL AND spd.deleted_at IS NULL AND DATE_FORMAT(sp.created_at,'%Y-%m-%d') <= fechaF 
